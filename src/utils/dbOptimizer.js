...
// Firestore 읽기/쓰기 최적화 유틸리티 - 🔥 v2.0 globalCacheService 통합
...
// ============================================
const TTL = {
  STATIC: 2 * 60 * 60 * 1000,    // 2시간 (거의 변경 안됨)
  SEMI_STATIC: 60 * 60 * 1000,   // 1시간 (가끔 변경)
  NORMAL: 30 * 60 * 1000,        // 30분 (자주 변경)
  FREQUENT: 10 * 60 * 1000,      // 10분 (매우 자주 변경)
};

// ============================================
// 캐시 관리 - globalCacheService 통합
// ============================================

class MemoryCache {
  constructor(defaultTTL = TTL.NORMAL) { // 🔥 [최적화 v2.0] 30분 (15분→30분)
    this.cache = new Map();
...
export async function getCachedDoc(path, options = {}) {
  const { ttl = TTL.NORMAL, forceRefresh = false } = options; // 🔥 [최적화 v2.0] 30분
