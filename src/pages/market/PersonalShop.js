...
const PersonalShop = () => {
  ...
  const loadSalesHistory = useCallback(async () => {
    ...
    try {
      ...
      // 🔥 자동 동기화 제거 (2026-05-20): 사용된 아이템(inventory에서 삭제됨)도
      // 거래 내역에는 영구히 남아 있어서, loadSalesHistory가 호출될 때마다
      // "inventory에 없음 → 누락" 으로 오판하여 자동 복원 → 사용한 아이템이 다시 나타남.
      // handlePurchase의 transaction.set이 atomic이라 누락 가능성이 거의 없음.
    } catch (error) {
