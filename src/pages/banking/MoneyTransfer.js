...
function MoneyTransfer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 예전엔 여기서 스위치를 껐다(한 번 쓰고 꺼짐). 2026-08-31 사용자 지시로 그만뒀다 —
      // 선생님이 끈 것이 아니라 **화면이 마음대로 끄는 것**이 불편했기 때문이다.
      // 끄고 싶으면 선생님이 직접 체크를 푼다. 그 선택은 이 화면에 있는 동안 유지된다.
      // ⚠️ 반복 회수를 막는 것은 이 초기화가 아니라 위의 확인창이다 — 그걸 지울 때는
      //    2026-07-27 사고를 다시 읽고 결정할 것.
    }
  };
