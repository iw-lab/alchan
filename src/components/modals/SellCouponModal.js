...
const SellCouponModal = memo(function SellCouponModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white p-5 rounded-lg max-w-[500px] w-[90%] max-h-[90vh] overflow-y-auto shadow-md border border-slate-200"
      ...
      >
        ...
        <div className="flex justify-between mb-4 p-2.5 bg-slate-50 rounded-md text-[15px] text-slate-700 border border-slate-200">
          <span>현재 보유 쿠폰:</span>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input
              ...
              min="1"
              max={currentCoupons}
              className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm outline-none bg-white text-slate-800 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition"
          ...
          {/* 에러 메시지 표시 */}
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-md px-4 py-3 mb-4 text-red-700 text-sm">
              <p className="m-0">{error}</p>
            </div>
          )}

          <div className="mb-5 px-4 py-2.5 bg-indigo-50 rounded-md border-l-4 border-indigo-500">
          ...
          <div className="flex justify-end gap-2.5 mt-5">
            <button
              type="button"
              onClick={handleCancel}
              className="px-4 py-2 bg-slate-200 text-slate-700 border-0 rounded-md text-sm cursor-pointer transition-colors duration-200 hover:bg-slate-300"
              disabled={isProcessing}
            >
              취소
            </button>
