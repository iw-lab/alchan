...
export function StatsDashboard({ userId, classCode, isAdmin = false }) {
    ...
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        weekData.push({
          ...
          color: i === 0 ? "bg-indigo-500" : "bg-indigo-600",
        });
      }
    ...
    return (
      <div className="animate-pulse space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-28 bg-gray-700 rounded-2xl" />
          ))}
        </div>
        <div className="h-40 bg-gray-700 rounded-2xl" />
      </div>
    );
  }
  ...
  const categoryData = [
    ...
    {
      label: "저축",
      value: stats.totalEarned - stats.totalSpent,
      color: "#3b82f6",
    },
  ];
