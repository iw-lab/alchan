...
const BASE_STYLE_BALD = `${STYLE_BASE.replace("isolated on PURE WHITE background (#ffffff), centered, no shadows on background, ", "")} Cute illustration of a BALD Korean elementary student base character (NO HAIR AT ALL, completely smooth shaved head, gender-neutral), standing pose front view. ${BODY_RULE} ${EYE_RULE} ${OUTLINE_RULE} Smooth bald scalp (light beige skin tone, no stubble, no shadow). ${OUTFIT_RULE} Friendly slight smile. Standing straight. Isolated on plain SOFT PASTEL solid background. Character CENTERED: TOP OF HEAD at 5%, CHIN at 25%, NECK at 27%, SHOULDERS at 30%, WAIST at 55%, KNEES at 78%, FEET at 95%. Full body visible.`;
