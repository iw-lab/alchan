...
const BASE_STYLE_BALD = `${STYLE_BASE.replace("isolated on PURE WHITE background (#ffffff), centered, no shadows on background, ", "")} Cute FULL BODY illustration of a BALD Korean elementary student base character (NO HAIR AT ALL, completely smooth shaved head, gender-neutral), STANDING POSE shown FROM TOP OF HEAD ALL THE WAY DOWN TO THE FEET (the entire body MUST be visible in the frame — NOT a bust portrait, NOT a head-and-shoulders crop, NOT a half-body shot). The character occupies the entire vertical space of the canvas. ${BODY_RULE} ${EYE_RULE} ${OUTLINE_RULE} Smooth bald scalp (light beige skin tone, no stubble, no shadow). ${OUTFIT_RULE} The character MUST be FULLY DRESSED with VISIBLE long-sleeve sweatshirt covering the torso and arms, AND VISIBLE long pants covering the legs down to the ankles, AND VISIBLE sneakers on the feet. NO bare chest, NO bare belly, NO bare legs, NO topless, NO underwear-only. Friendly slight smile. Standing straight. Isolated on plain SOFT PASTEL solid background. Character CENTERED with exact proportions: TOP OF HEAD at vertical 5%, CHIN at 25%, NECK at 27%, SHOULDERS at 30%, WAIST at 55%, KNEES at 78%, FEET at 95%. FULL BODY MUST BE VISIBLE in frame.`;
