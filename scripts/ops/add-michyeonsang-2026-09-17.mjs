#!/usr/bin/env node
/**
 * platformApps/_registry 에 «미현상(1인칭 방탈출)» 하나만 덧붙인다 (2026-09-17).
 *
 * 🔴 add-teeshot-2026-09-17.mjs 와 같은 «덧붙이기» 방식이다. 통짜 시드(seed-app-registry)를
 *    쓰면 정본에만 있는 owner 필드가 조용히 날아간다.
 * 🔴 링크 등재만 한다. `platformAppPolicies/{id}.launchUrl` 은 건드리지 않는다 — AAP 토큰 연동이
 *    아니라 그냥 학습사이트 링크다.
 *
 * 실행: node scripts/ops/add-michyeonsang-2026-09-17.mjs [--dry]
 */
