...
//
//   교사 화면(P1-5)이 붙기 전까지 이 스크립트가 그 자리를 메웠다.
//
// 지금의 위상 (2026-08-22 · P1-5 배선 후)
//   교사는 앱 안 `/admin/learning-apps` 에서 본다. 이 스크립트는 **교사가 못 보는 것**을 보는
//   자리로 남는다: 전 학급 횡단 조회, `sub`·`jti`·`clientRunId` 같이 화면 투영에서 일부러
//   뺀 필드, 그리고 앱이 뜨지 않을 때의 진단. 지우지 않는다 — 화면이 죽으면 이게 유일한 창이다.
//
// 용법
