...
라이브 확인
- **클라이언트**: 라이브 사이트에서 교사 세션·국고·쿠폰 28개·거래내역(배당·주급·경매)이 그대로 렌더.
- **Cloud Functions**: 24:00 KST 정시 `hourlySchedulerV2` 가 서울 DB 로 **읽고 썼다** —
  5개 학급 86명 일일 과제 리셋, `systemState/lastMidnightReset` 갱신(15:00:03Z), 오류 0.
  23:00 KST 실행은 프리즈로 건너뛴 것이 로그에 그대로 보인다(의도한 대로).
...
- **양쪽 다 잃는 경우**: 로컬 JSONL 덤프로 재적재 — **2026-08-20 삭제 완료**(학생 PII 45MB).
  삭제 전 `alchan-kr` 사본이 61,657건 그대로임을 집계로 재확인했고(라이브는 정상 활동으로 +7건),
  `(default)` 에 일 단위 백업(보존 7일)도 걸려 있다.
