...
| # | 항목 | 상태 | 근거 |
|---|---|---|---|
| P1-1 | AAP 토큰 발급 CF + JWKS (P1-8 토큰 위생 포함) | 🔄 FULL 검증 반영 완료·배포 대기 | `functions/aap/*` · `docs/AAP_V1_SPEC.md` · 테스트 37개 · 변이 15개 전부 검출 · 정책문서 11개 시드 + kill switch 왕복 시험 |
| P1-7 | 서버 소유 achievement 카탈로그 | ⬜ | **P1-2 의 선행조건** — 클라가 금액을 보내면 캡이 가격표가 된다 |
