...
const GLOBAL_CEILING = Object.freeze({
  /** 한 학생이 **모든 앱을 합쳐** 하루에 받을 수 있는 현금 (주급의 5%) */
  STUDENT_CASH_PER_DAY: Math.round(SALARY.BASE * 0.05), // 100,000
  /** 한 학생이 모든 앱을 합쳐 하루에 받을 수 있는 쿠폰 */
  STUDENT_COUPON_PER_DAY: 10,
  /** 한 학급 전체가 하루에 받을 수 있는 현금 (= 1인 주급) — 폭주 차단기 */
  CLASS_CASH_PER_DAY: SALARY.BASE, // 2,000,000
  /** 한 학급 전체가 하루에 받을 수 있는 쿠폰 */
  CLASS_COUPON_PER_DAY: 200,
  /** 한 앱이 하루에 발행할 수 있는 현금 총액 — 앱이 털렸을 때의 차단기 */
  APP_CASH_PER_DAY: SALARY.BASE * 2, // 4,000,000
  /** 한 앱이 하루에 발행할 수 있는 쿠폰 총량 */
  APP_COUPON_PER_DAY: 400,
});
