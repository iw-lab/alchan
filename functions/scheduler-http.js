...
exports.toggleVacationMode = onCall({ region: "asia-northeast3" }, async (request) => {
  const { uid, isSuperAdmin, userData } = await checkAuthAndGetUserData(request, true); // 관리자만 실행 가능

  // 슈퍼관리자 체크 (role이 'super_admin'인 경우만)
  if (!isSuperAdmin) {
    throw new HttpsError('permission-denied', '슈퍼관리자만 방학 모드를 설정할 수 있습니다.');
  }
  ...
  try {
    ...
    logger.info(`[toggleVacationMode] 방학 모드 ${enabled ? 'ON' : 'OFF'} by ${userData?.name || uid}`);

    return {
      success: true,
      vacationMode: enabled === true,
      message: enabled ? '방학 모드가 활성화되었습니다. 스케줄러가 중지됩니다.' : '방학 모드가 해제되었습니다. 스케줄러가 다시 작동합니다.'
    };
  } catch (error) {
...
// 🔥 방학 모드 상태 조회 API
exports.getVacationModeStatus = onCall({ region: "asia-northeast3" }, async (request) => {
